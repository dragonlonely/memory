	...
	<div class="panel-body">
	    ...
	    <div class="row ${pageBean.totalCount<=0?"div1":"" }">
	    	...
	    	<c:forEach items="${pageBean.data}" var="g" varStatus="i">
		    	<div class="col-sm-3">
				    <div class="thumbnail">
				      <img src="${g.secretpicture}" style="width:180px;height:200px;border-radius:5px"  alt="" />
				      <div class="caption">
				        <h4>图片名称:<a href="${pageContext.request.contextPath}/memory/getsecretpicturesdetailbyid.action?id=${g.id}">${g.secretname}</a></h4>
						  <p>热度:
							  <c:forEach begin="1" end="${g.star}">
								  <img src="${pageContext.request.contextPath}/image/star_red.gif" alt="star"/>
							  </c:forEach>
						  </p>
						  <p>添加日期:${g.createdate}</p>
				      </div>
				    </div>
				  </div>
	    	</c:forEach>
